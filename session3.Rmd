...
We can now have a list of genes ordered according to their evidence for being differentially-expressed. You should have saved a `results_cellType.rds` object in the previous session. If you don't have it you can download using [this link](Robjects/results_cellType.rds).
...
As the results table no longer contains read counts, we will have to load these from a previous object. [Click here to download if you are missing this](Robjects/dds.rds).
...
- Repeat the same heatmap as above, but for the top 100 most differentially-expressed genes **between lactation and virgin**
    + Load your `de_status.rds` file from last week. If you don't have one, [click here to download one](Robjects/results_status.rds)
